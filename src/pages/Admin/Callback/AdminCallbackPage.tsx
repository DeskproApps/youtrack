...
const AdminCallbackPage: FC = () => {
  ...
  useInitialisedDeskproAppClient(async (client) => {
    const oauth2 = await client.startOauth2Local(
      ({ callbackUrl, state }) => {
        return `https://${context?.settings.instance_url}/api/rest/oauth2/auth?${createSearchParams([
          ["response_type", "code"],
          ["client_id", "xxx"],
          ["state", state],
  ...
  return (
    <>
      ...
      <Description>The callback URL will be required during the YouTrack app setup</Description>
    </>
  );
};
