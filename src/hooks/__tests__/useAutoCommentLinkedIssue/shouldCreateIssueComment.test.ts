...
jest.mock("@deskpro/app-sdk", () => ({
  ...
  useDeskproLatestAppContext: () => ({
    context: {
      settings: { dont_add_comment_when_linking_issue: false },
      data: {
...
describe("useAutoCommentLinkedIssue", () => {
  ...
  test("should to called the service to create an automatic comment (link issue)", async () => {
    ...
    expect(createIssueCommentService).toHaveBeenCalled();
  });
